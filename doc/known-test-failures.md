...
inferencing gap: it wants a base-type query to return subclasses with the flag **off**, which
contradicts `SparqlLinqExemplarTest.FiltersToExactType`. ADR-0045 settled the flag-*on* half; the
flag-off half is still an open semantics choice.

Of these, the LINQ entry is an open design decision. **The blank-node entry is a genuine defect** — the
only quarantined case that is. No quarantined test is a missing LINQ translation or a datatype bug.
...
### Why in-memory inferencing did not work (diagnosed on dotNetRDF 3.5.2) — **fixed in ADR-0045**
Kept because the diagnosis was right and the shape recurs. Three independent gaps, any one of
